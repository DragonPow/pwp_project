...
frappe.ui.form.on('Digital Signature', {
    refresh: function(frm) {
        if (frm.doc.verification_status === 'Pending') {
            frm.add_custom_button(__('Verify Signature'), function() {
                frappe.call({
                    method: 'electronic_office.electronic_office.doctype.digital_signature.digital_signature.verify_signature',
                    args: {
        ...
        if (frm.doc.verification_status === 'Verified') {
            frm.add_custom_button(__('Revoke Signature'), function() {
                frappe.prompt([
                    ...
                    frappe.call({
                        method: 'electronic_office.electronic_office.doctype.digital_signature.digital_signature.revoke_signature',
                        args: {
                            docname: frm.doc.name,
    ...
    document: function(frm) {
        if (frm.doc.document) {
            ...
            frappe.call({
                method: 'electronic_office.electronic_office.doctype.document.document.get_latest_version',
                args: {
