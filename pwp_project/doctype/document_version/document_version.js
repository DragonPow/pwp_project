...
frappe.ui.form.on('Document Version', {
    refresh: function(frm) {
        ...
        if (!frm.doc.is_current) {
            frm.add_custom_button(__('Set as Current'), function() {
                frappe.confirm(
                    ...
                    function() {
                        frappe.call({
                            method: 'pwp_project.pwp_project.doctype.document_version.document_version.set_as_current',
                            args: {
        ...
        frm.add_custom_button(__('Compare with Previous'), function() {
            frappe.call({
                method: 'pwp_project.pwp_project.doctype.document_version.document_version.get_previous_version',
                args: {
                    docname: frm.doc.name
                },
                callback: function(r) {
            ...
            if (!r.exc && r.message) {
                let dialog = new frappe.ui.Dialog({
                    ...
                    primary_action: function() {
                        ...
                        frappe.call({
                            method: 'pwp_project.pwp_project.doctype.document_version.document_version.compare_with_version',
                            args: {
                                docname: frm.doc.name,
