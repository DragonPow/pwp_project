    ...
    refresh: function(frm) {
        if (frm.doc.status === 'Pending') {
            frm.add_custom_button(__('Start Task'), function() {
                frappe.call({
                    method: 'pwp_project.pwp_project.doctype.task.task.start_task',
                    args: {
        ...
        if (frm.doc.status === 'In Progress') {
            frm.add_custom_button(__('Complete Task'), function() {
                frappe.call({
                    method: 'pwp_project.pwp_project.doctype.task.task.complete_task',
                    args: {
                        docname: frm.doc.name
                    },
                    callback: function(r) {
            ...
            frm.add_custom_button(__('Cancel Task'), function() {
                frappe.prompt([
                    ...
                    frappe.call({
                        method: 'pwp_project.pwp_project.doctype.task.task.cancel_task',
                        args: {
                            docname: frm.doc.name,
        ...
        if (frm.doc.assigned_to) {
            frm.add_custom_button(__('Reassign Task'), function() {
                frappe.prompt([
                    ...
                    frappe.call({
                        method: 'pwp_project.pwp_project.doctype.task.task.reassign_task',
                        args: {
                            docname: frm.doc.name,
