    ...
    refresh: function(frm) {
        ...
        frm.add_custom_button(__('Create Version'), function() {
            frappe.prompt([
                ...
                frappe.call({
                    method: 'pwp_project.pwp_project.doctype.document.document.create_version',
                    args: {
        ...
        frm.add_custom_button(__('View Versions'), function() {
            frappe.call({
                method: 'pwp_project.pwp_project.doctype.document.document.get_versions',
                args: {
                    docname: frm.doc.name
                },
                callback: function(r) {
...
function restore_version(version_name) {
    frappe.confirm(
        ...
        function() {
            frappe.call({
                method: 'pwp_project.pwp_project.doctype.document_version.document_version.restore_to_document',
                args: {
...
function show_export_dialog(frm) {
    let dialog = new frappe.ui.Dialog({
        ...
        primary_action: function() {
            ...
            frappe.call({
                method: 'pwp_project.pwp_project.doctype.document.document.export_document',
                args: {
                    docname: frm.doc.name,
...
function show_grant_access_dialog(frm) {
    let dialog = new frappe.ui.Dialog({
        ...
        primary_action: function() {
            let values = dialog.get_values();
            frappe.call({
                method: 'pwp_project.pwp_project.doctype.document.document.grant_temporary_access',
                args: {
                    docname: frm.doc.name,
                    user: values.user,
